-- disconnected.lua - Part 1/4: Core Setup & Helpers
local Library = {}
...
-- disconnected.lua - Part 2/4: Font, Holders, Snow, Notifications
...
function Library:Connect(event, cb)
    ...
    TableInsert(self.Connections, { Event = event, Callback = cb, Connection = conn })
    return conn
end
...
-- disconnected.lua - Part 3/4: Components & UI Elements
...
function Instances:Tween(info, goal) return TweenService:Create(self.Instance, info or TweenInfo.new(TweenTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), goal) end
...
function Library:Window(data)
    ...
    local win = BuildWindow({
        ...
        Position = UDim2.new(0, Camera.ViewportSize.X / 3.3, 0, Camera.ViewportSize.Y / 3.3),
...
-- disconnected.lua - Part 4/4: Toggle, Mobile Button & Return
